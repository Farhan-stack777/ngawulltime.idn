...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($imageTmpName, $uploadFile)) {
            ...
            header("Location:tas.php?error=image_upload_failed");
            exit();
        }
    }
    ...
    $stmt = $conn->prepare("INSERT INTO tb_tas (gambar, judul, harga, link) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $gambar, $judul, $harga, $link);

    if ($stmt->execute()) {
        header("Location:tas.php?success=data_saved");
    } else {
        header("Location:tas.php?error=db_error");
    }

    $stmt->close();
    $conn->close();
} else {
    header("Location:tas.php?error=form_not_submitted");
}
?>